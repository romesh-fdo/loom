...
<form @if ($formId) id="{{ $formId }}" @endif
      ...
      action="{{ $record ? route('loom.page-blocks.update', $record) : route('loom.page-blocks.store') }}"
    ...
    <div class="loom-form-actions">
        ...
        <a href="{{ route('loom.page-blocks.index') }}" class="loom-form-btn loom-form-btn--secondary">Cancel</a>
        @if ($record)
            <button type="submit" form="delete-record-form" class="loom-form-btn loom-form-btn--danger ms-auto" onclick="return confirm('Delete this record?')">Delete</button>
        @endif
    </div>
</form>

@if ($record)
    <form id="delete-record-form" method="POST" action="{{ route('loom.page-blocks.destroy', $record) }}" class="d-none">
        @csrf
        @method('DELETE')
    </form>
@endif
