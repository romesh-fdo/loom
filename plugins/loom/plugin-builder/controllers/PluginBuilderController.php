...
namespace Loom\PluginBuilder\Controllers;
...
class PluginBuilderController extends Controller
{
    ...
    {
        $installed = app(PluginManager::class)->getPlugins();

        $plugins = collect($installed)
            ->reject(fn ($plugin) => $plugin->getName() === 'plugin-builder')
            ->map(function ($plugin) {
